...
import { searchCustomersClient, createCustomerClient } from '@entities/customer/api.client';
...
export function CustomerAutocomplete({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const rows = await searchCustomersClient(
          organizationId,
          q === '' ? undefined : q,
          10,
          controller.signal,
        ...
        if ((e as Error).name !== 'AbortError') {
          ...
          toast.error('Could not search customers', msg);
          setOptions([]);
        }
      } finally {
        setLoading(false);
  ...
  const select = useCallback((c: Customer) => {
    setSelected(c);
    setText(c.name);
    setOpen(false);
    onSelect?.(c);
  ...
  const create = async () => {
    ...
    try {
      const key = typeof crypto !== 'undefined' && 'randomUUID' in crypto
        ? crypto.randomUUID()
        : undefined;
      const { id } = await createCustomerClient(organizationId, { name }, key);
      const created: Customer = { id, name, email: null, phone: null, createdAt: new Date().toISOString() };
      select(created);
      toast.success('Customer created', name);
    } catch (e) {
      const msg = e instanceof ApiError ? (e.problem.detail ?? e.problem.title) : (e as Error).message;
      toast.error('Could not create customer', msg);
    } finally {
      setCreating(false);
    }
  };
        ...
        {open && !selected && (
          ...
          >
            ...
            {!loading && options.length === 0 && text.trim() === '' && (
              <li className="px-3 py-2 text-xs text-slate-500">Type to search customers.</li>
            )}
            ...
            {canCreate && (
              <li>
                ...
                >
                  ...
                  <span>
                    Create customer &ldquo;<strong>{text.trim()}</strong>&rdquo;
                  </span>
                </button>
              </li>
            )}
